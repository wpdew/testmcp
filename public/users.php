...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    ...
    if (strlen($username) < 3) {
        ...
        $alertType = 'danger';
    } elseif ($db->fetch('SELECT * FROM users WHERE username = ?', [$username])) {
...
if (isset($_GET['delete'])) {
    ...
    if ($user && $user['username'] !== 'admin') {
        ...
        $alert = 'Нельзя удалить пользователя admin.';
        $alertType = 'danger';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit'])) {
    ...
    if (strlen($username) < 3) {
        $alert = 'Логин должен быть не менее 3 символов.';
        $alertType = 'danger';
    } elseif ($db->fetch('SELECT * FROM users WHERE username = ? AND id != ?', [$username, $id])) {
        $alert = 'Пользователь с таким логином уже существует.';
        $alertType = 'danger';
    } else {
...
$users = $db->fetchAll("SELECT * FROM users $where ORDER BY id ASC", $params);
$editUser = null;
if (isset($_GET['edit'])) {
    $editUser = $db->fetch('SELECT * FROM users WHERE id = ?', [(int)$_GET['edit']]);
}
?>
<!DOCTYPE html>
<html lang="ru">
                    ...
                    <div class="card-body">
                        <div class="row g-2 mb-3 align-items-center">
                            ...
                            <div class="col-md-1 text-end">
                                <button type="button" class="btn btn-success w-auto px-2 d-flex align-items-center justify-content-center" data-bs-toggle="modal" data-bs-target="#addUserModal" title="Добавить пользователя">
